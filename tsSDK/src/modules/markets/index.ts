...
import { MARKETS, MarketConfig as ConfigMarketConfig } from "configs/markets";
...
export class Markets extends Module {
  ...
  async getMarkets(offset = 0n, limit = 100n): Promise<MarketsResult> {
    ...
    const chainId = this.chainId;
    const configMarkets = MARKETS[chainId] ?? {};
    const userConfigMarkets = this.sdk.config.markets ?? {};

    // Use markets from local config instead of Oracle API
    const marketsResult = Object.entries(configMarkets).reduce(
      (
        acc: MarketsResult,
        [marketTokenAddress, marketConfig]: [string, ConfigMarketConfig]
      ) => {
        try {
          // Check if market is disabled in user config
          if (userConfigMarkets[marketTokenAddress]?.isListed === false) {
            return acc;
          }

          const indexToken = getToken(chainId, convertTokenAddress(chainId, marketConfig.indexTokenAddress, "native"));
          const longToken = getToken(chainId, marketConfig.longTokenAddress);
          const shortToken = getToken(chainId, marketConfig.shortTokenAddress);

          const isSameCollaterals = marketConfig.longTokenAddress === marketConfig.shortTokenAddress;
          const isSpotOnly = marketConfig.indexTokenAddress === zeroAddress;
          ...
          acc.marketsData![marketTokenAddress] = {
            marketTokenAddress: marketConfig.marketTokenAddress,
            indexTokenAddress: marketConfig.indexTokenAddress,
            longTokenAddress: marketConfig.longTokenAddress,
            shortTokenAddress: marketConfig.shortTokenAddress,
            isSameCollaterals,
            isSpotOnly,
            name,
            data: "",
          ...
          // eslint-disable-next-line no-console
          console.warn(`Unsupported market ${marketTokenAddress}`, e);
        }
