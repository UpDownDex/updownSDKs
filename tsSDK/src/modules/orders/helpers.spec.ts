...
describe("increaseOrderHelper", () => {
  ...
  describe("validation", () => {
    ...
    it("should throw an error if wrong collateral token selected", async () => {
      ...
      await expect(e).toBe(
        "Invalid collateral token. Only long WETH and short USDC tokens are available."
      );
    });
  });
  ...
  describe("parameters", () => {
    beforeEach(() => {
      ...
      createIncreaseOrderSpy = vi
        .spyOn(arbitrumSdk.orders, "createIncreaseOrder")
        .mockResolvedValue({ hash: "0x" } as any);
    });
    ...
    it("should call createIncreaseOrder with correct parameters for a market order with payAmount", async () => {
      ...
      const getIncreasePositionAmountsSpy = vi.spyOn(
        tradeAmounts,
