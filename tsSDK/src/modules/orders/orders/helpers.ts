...
async function getAndValidateBaseParams(
  ...
  const [marketsInfoResult, uiFeeFactor, gasPrice, gasLimits] =
    await Promise.all([
      passThoughOrFetch(
        {
...
export async function increaseOrderHelper(
  ...
  const { tokensData, marketsInfoData, uiFeeFactor, gasLimits, gasPrice } =
    await getAndValidateBaseParams(sdk, params);
  ...
  if (collateralToken.isSynthetic) {
    throw new Error(
      "collateralTokenAddress: synthetic tokens are not supported"
    );
  }
  ...
  const payOrSizeAmount =
    "payAmount" in params ? params.payAmount : params.sizeAmount;
  ...
  const createIncreaseOrderParams: Parameters<
    typeof sdk.orders.createIncreaseOrder
  >[0] = {
    marketsInfoData,
    tokensData,
    isLimit,
...
function getTriggerRatio({
  ...
  if (!fromToken.prices || !toToken.prices) {
    throw new Error("Token prices are required for trigger ratio calculation");
  }
...
export async function swap(sdk: UpdownSdk, params: SwapParams) {
  const { tokensData, marketsInfoData, uiFeeFactor, gasLimits, gasPrice } =
    await getAndValidateBaseParams(sdk, params);
  ...
  if (isWrapOrUnwrap) {
    // For wrap/unwrap, use createWrapOrUnwrapOrder instead
    const tokenAmount =
      "fromAmount" in params ? params.fromAmount : params.toAmount;
    const isWrap = Boolean(getIsWrap(fromToken, toToken));

    return sdk.orders.createWrapOrUnwrapOrder({
      amount: tokenAmount,
      isWrap,
    });
  }
  ...
  if ("fromAmount" in params) {
    swapAmounts = getSwapAmountsByFromValue({
      ...
      allowedSwapSlippageBps: isLimit
        ? BigInt(params.allowedSlippageBps ?? 100)
        : undefined,
      marketsInfoData,
      chainId: sdk.chainId,
      externalSwapQuoteParams: undefined,
  ...
  const createSwapOrderParams: Parameters<
    typeof sdk.orders.createSwapOrder
  >[0] = {
    tokensData,
