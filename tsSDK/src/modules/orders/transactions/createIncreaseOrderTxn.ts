...
export type SecondaryDecreaseOrderParams = DecreaseOrderParams &
...
export async function createIncreaseOrderTxn({
  ...
  const totalWntAmount = concat<
    undefined | SecondaryDecreaseOrderParams | SecondaryUpdateOrderParams
  >(createDecreaseOrderParams, updateOrderParams).reduce(
    (acc, p) => (p ? acc + p.executionFee : acc),
    wntAmountToIncrease,
  )
  ...
  const cancelEncodedPayload = createCancelEncodedPayload(
    cancelOrderParams?.map(({ orderKey }) => orderKey) || [],
  )
  ...
  const finalPayload = [
    ...encodedPayload,
    ...decreaseEncodedPayload,
    ...cancelEncodedPayload,
