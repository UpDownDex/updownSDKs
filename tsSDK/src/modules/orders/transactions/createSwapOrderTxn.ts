...
export async function createSwapOrderTxn(sdk: UpdownSdk, p: SwapOrderParams) {
...
async function getParams(sdk: UpdownSdk, p: SwapOrderParams) {
  ...
  return {
    minOutputAmount,
    totalWntAmount,
    encodedPayload: multicall.filter(Boolean).map((call) =>
      encodeFunctionData({
