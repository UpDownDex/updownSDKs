...
export class Positions extends Module {
  ...
  private getKeysAndPricesParams(p: { marketsData: MarketsData | undefined; tokensData: TokensData | undefined }) {
    ...
    for (const market of markets) {
      const marketPrices = getContractMarketPrices(tokensData, market, this.chainId);
  ...
  async getPositions(p: {
    ...
    // Debug: Log marketsKeys status
    if (keysAndPrices.marketsKeys.length === 0) {
      // eslint-disable-next-line no-console
      console.warn(
        `⚠️  No markets with prices found. marketsKeys is empty. ` +
        `This means getContractMarketPrices returned undefined for all markets. ` +
        `Falling back to getAccountPositions (doesn't require prices).`
      );
    } else {
      // eslint-disable-next-line no-console
      // console.log(
      //   `✅ Found ${keysAndPrices.marketsKeys.length} markets with prices. ` +
      //   `Using getAccountPositionInfoList.`
      // );
    }

    // If marketsKeys is empty, use getAccountPositions which doesn't require prices
    if (keysAndPrices.marketsKeys.length === 0) {
      // Fallback to getAccountPositions (doesn't require market prices)
      const request = {
        reader: {
      ...
      const positions = await this.sdk.executeMulticall(request).then((res) => {
        ...
        return positions.reduce((positionsMap: PositionsData, position: any) => {
          ...
          positionsMap[positionKey] = {
            ...
            pendingImpactAmount: 0n, // Not available in getAccountPositions
            isLong: flags.isLong,
            pendingBorrowingFeesUsd: 0n, // Not available in getAccountPositions
            fundingFeeAmount: 0n, // Not available in getAccountPositions
            claimableLongTokenAmount: 0n, // Not available in getAccountPositions
            claimableShortTokenAmount: 0n, // Not available in getAccountPositions
            pnl: 0n, // Not available in getAccountPositions
            positionFeeAmount: 0n, // Not available in getAccountPositions
            traderDiscountAmount: 0n, // Not available in getAccountPositions
            uiFeeAmount: 0n, // Not available in getAccountPositions
            data: "",
          };
      ...
      const positionsData = this.getPositionsData({
        ...
        allPositionsKeys: undefined, // Not available when using getAccountPositions
      });
    ...
    // Use getAccountPositionInfoList when we have market prices
    const request = {
      reader: {
        contractAddress: getContract(chainId, "SyntheticsReader"),
        abiId: "SyntheticsReader",
    ...
    // Debug: Log positionsData status
    const positionsDataKeys = positionsData ? Object.keys(positionsData) : [];
    // eslint-disable-next-line no-console
    console.log(
      `📊 getAccountPositionInfoList result: ${positionsDataKeys.length} positions ` +
      `(raw positions: ${Object.keys(positions).length})`
    );

    // If getAccountPositionInfoList returned no positions, try getAccountPositions as fallback
    // This can happen if marketsKeys doesn't include all markets where positions exist
    if (!positionsData || Object.keys(positionsData).length === 0) {
      // eslint-disable-next-line no-console
      console.log(
        `⚠️  getAccountPositionInfoList returned no positions. ` +
        `Trying fallback: getAccountPositions (doesn't require prices)...`
      );
      try {
        ...
        const fallbackPositions = await this.sdk.executeMulticall(fallbackRequest).then((res) => {
          const positions = res.data.reader.positions.returnValues;
        ...
        if (fallbackPositions && Object.keys(fallbackPositions).length > 0) {
          // eslint-disable-next-line no-console
          console.warn(
            `⚠️  getAccountPositionInfoList returned no positions, but getAccountPositions found ${Object.keys(fallbackPositions).length} positions. ` +
            `Using fallback results. This might indicate a mismatch between marketsKeys and actual positions.`
          );
          
          const fallbackPositionsData = this.getPositionsData({
            positionsData: fallbackPositions,
            allPositionsKeys: undefined,
          });

          return {
            positionsData: fallbackPositionsData,
          };
        }
      } catch (fallbackError) {
        // eslint-disable-next-line no-console
        console.warn(`⚠️  Fallback to getAccountPositions failed: ${fallbackError}`);
      }
    }
  ...
  async getPositionsInfo(p: {
    ...
    const positionsInfoData = Object.keys(positionsData).reduce((acc: PositionsInfoData, positionKey: string) => {
      ...
      if (!marketInfo || !indexToken || !pnlToken || !collateralToken || !longToken || !shortToken) {
        // Debug: Log why position was skipped
        // eslint-disable-next-line no-console
        console.warn(
          `⚠️  Position ${positionKey} skipped in getPositionsInfo: ` +
          `marketInfo=${!!marketInfo}, indexToken=${!!indexToken}, ` +
          `pnlToken=${!!pnlToken}, collateralToken=${!!collateralToken}, ` +
          `longToken=${!!longToken}, shortToken=${!!shortToken}`
        );
        return acc;
      }
