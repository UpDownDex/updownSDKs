...
import { NATIVE_TOKEN_ADDRESS, getToken, getTokens, getTokensMap, getV2Tokens, getWrappedToken } from "configs/tokens";
...
export class Tokens extends Module {
  ...
  async getTokensData(): Promise<TokensDataResult> {
    ...
    // Get tokens from local config instead of Oracle API
    const configTokens = (getTokens(this.chainId) || []).filter(
      (token) => token.address && token.address !== "<market-token-address>" && token.address.startsWith("0x")
    );
    const { pricesData, updatedAt: pricesUpdatedAt } = await this.getTokenRecentPrices();

    const balancesData = this.account ? await this.getTokensBalances(this.account, configTokens) : ({} as TokenBalancesData);
    ...
    return {
      tokensData: configTokens.reduce((acc: TokensData, token) => {
