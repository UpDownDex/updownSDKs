...
export async function fetchTradeActions({
    ...
    AND: [
          ...
          // For non-swap orders
          {
            AND: !hasNonSwapRelevantDefinedMarkets
              ...
              : [
                  ...
                  {
                    OR: nonSwapRelevantDefinedFiltersLowercased.map(
                      (filter) => ({
                        marketAddress_eq:
                          filter.marketAddress === 'any'
                            ? undefined
                            : filter.marketAddress,
                        isLong_eq:
              ...
              : [
                  ...
                  },
                  {
                    OR: [
                      ...
                      swapRelevantDefinedMarketsLowercased.map(
                        (marketAddress) => ({
      ...
      },
      {
        OR: orderEventCombinations?.map((combination) => {
          ...
          if (
            ...
            [OrderType.MarketDecrease, OrderType.MarketIncrease].includes(
              combination.orderType,
            )
          ) {
      ...
      },
      {
        ...
        OR: [
          ...
          { eventName_not_eq: TradeActionType.OrderCreated },
        ],
      },
    ],
  })
  ...
  const result = await graphqlFetcher<{ tradeActions: SubsquidTradeAction[] }>(
    endpoint,
    query,
  )
  ...
  const transformer = createRawTradeActionTransformer(
    marketsInfoData,
    wrappedToken,
    tokensData,
  )

  let tradeActions = rawTradeActions
    .map(transformer)
  ...
  if (hasCollateralFilter) {
    tradeActions = tradeActions.filter((tradeAction) => {
      ...
      const desiredCollateralAddresses =
        collateralFilterTree[positionTradeAction.isLong ? 'long' : 'short']?.[
          positionTradeAction.marketAddress
        ]
        ...
        if (!marketsInfoData) {
          ...
          const { outTokenAddress } = getSwapPathOutputAddresses({
            ...
            initialCollateralAddress:
              positionTradeAction.initialCollateralTokenAddress,
            isIncrease: isIncreaseOrderType(tradeAction.orderType),
            shouldUnwrapNativeToken:
              positionTradeAction.shouldUnwrapNativeToken,
          ...
          collateralMatch =
            ...
            Boolean(desiredCollateralAddresses?.[outTokenAddress as Address])
        }
      } else if (isTriggerDecreaseOrderType(tradeAction.orderType)) {
        collateralMatch = Boolean(
          desiredCollateralAddresses?.[
            positionTradeAction.initialCollateralTokenAddress
          ],
        )
      }
