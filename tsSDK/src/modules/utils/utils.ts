import {
  ...
  EXECUTION_FEE_MAX_GAS_PRICE_WEI,
  EXECUTION_FEE_PRIORITY_WEI,
  EXECUTION_FEE_USE_BASE_FEE,
...
export class Utils extends Module {
    ...
    switch (tradeFeesType) {
      ...
      case "increase": {
        ...
        return estimateExecuteIncreaseOrderGasLimit(gasLimits, {
          swapsCount: increaseAmounts.swapStrategy.swapPathStats?.swapPath.length ?? 0,
        });
      }
      case "decrease": {
  ...
  async getGasPrice() {
    ...
    const useBaseFee = EXECUTION_FEE_USE_BASE_FEE[this.chainId];

    if (useBaseFee) {
      for (const tag of ["pending", "latest"] as const) {
        try {
          const block = await withRetry(
            () => this.sdk.publicClient.getBlock({ blockTag: tag }),
            {
              retryCount: 2,
              shouldRetry: ({ error }) =>
                !!error?.message?.includes("invalid value for value.hash"),
            }
          );
          const baseFee = block?.baseFeePerGas;
          if (baseFee != null && baseFee > 0n) {
            const bps = executionFeeConfig?.defaultBufferBps ?? 0;
            const buffer = (baseFee * BigInt(bps)) / 10000n;
            const priority = EXECUTION_FEE_PRIORITY_WEI[this.chainId] ?? 2n * 10n ** 9n;
            return baseFee + buffer + priority;
          }
        } catch {
          // try next tag or fallback to estimateFeesPerGas
        }
      }
    }
    ...
    let price = gasPrice + premium;

    const maxGasPrice = EXECUTION_FEE_MAX_GAS_PRICE_WEI[this.chainId];
    if (maxGasPrice != null && price > maxGasPrice) price = maxGasPrice;
