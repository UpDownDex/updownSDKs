...
export async function getGasLimit(
  ...
  try {
    ...
    // Simulate as the same `from` as estimateGas. publicClient has no
    // `client.account`; omitting `account` uses address(0) and multicalls
    // that pull ERC20 via transferFrom(msg.sender) revert misleadingly.
    const callParams = {
      to: contractAddress,
      data,
      value: BigInt(value),
...
export async function callContract(
  ...
  const txnCalls = clients.map(async (client) => {
    ...
    async function retrieveGasLimit() {
      if (opts.gasLimit) {
        return BigInt(opts.gasLimit)
      }
      const fromAccount =
        (sdk.walletClient?.account?.address as Address | undefined) ??
        (sdk.config.account as Address | undefined)
      if (!fromAccount) {
        throw new Error(
          'Account is required for gas estimation (set config.account or walletClient.account)',
        )
      }
      return getGasLimit(
        client,
    ...
    const gasPriceDataPromise = getGasPrice(sdk.publicClient, sdk.chainId).then(
      (gasPriceData) => {
