import {
  ContractsChainId,
  EXECUTION_FEE_BUFFER_BPS,
  EXECUTION_FEE_MAX_TOKEN_AMOUNT,
  getExcessiveExecutionFee,
  getHighExecutionFee,
  MIN_EXECUTION_FEE_USD,
} from 'configs/chains';
...
export function getExecutionFee(
  ...
  const maxFee = EXECUTION_FEE_MAX_TOKEN_AMOUNT[chainId as ContractsChainId];
  if (maxFee != null && feeTokenAmountPerExecution > maxFee) {
    feeTokenAmountPerExecution = maxFee;
  }

  let feeTokenAmount = feeTokenAmountPerExecution * BigInt(numberOfParts ?? 1);
  const bufferBps = EXECUTION_FEE_BUFFER_BPS[chainId as ContractsChainId];
  if (bufferBps != null && bufferBps > 0) {
    feeTokenAmount = (feeTokenAmount * BigInt(10000 + bufferBps)) / 10000n;
  }
...
export function estimateExecuteIncreaseOrderGasLimit(
  ...
  const result = gasLimits.increaseOrder + gasPerSwap * swapsCount + (order.callbackGasLimit ?? 0n);
