...
export function getContractMarketPrices(
  ...
  // Convert indexToken address to native format (consistent with getMarketsInfo)
  // This ensures we can find the token even if market.indexTokenAddress is in wrapped format
  const indexTokenAddress = chainId
