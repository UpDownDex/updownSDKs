...
export class Multicall {
  ...
  static async getInstance(sdk: UpdownSdk) {
  ...
  async call(request: MulticallRequestConfig<any>, maxTimeout: number) {
    ...
    contractKeys.forEach((contractKey) => {
      ...
      Object.keys(contractCallConfig.calls).forEach((callKey) => {
        ...
        abis[contractCallConfig.contractAddress] = abis[
          contractCallConfig.contractAddress
        ] || [
        ...
        (
          {
            ...
            status,
            error,
          }: { result: any; status: string; error: any },
          i: number,
        ) => {
          ...
          if (status === 'success') {
            ...
            multicallResult.data[contractKey] =
              multicallResult.data[contractKey] || {}
            ...
            multicallResult.errors[contractKey] =
              multicallResult.errors[contractKey] || {}
    ...
    const result = await Promise.race([
      ...
      sleep(maxTimeout, timeoutController.signal).then(() =>
